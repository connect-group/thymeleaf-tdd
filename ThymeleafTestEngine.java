...
public class ThymeleafTestEngine extends SpringThymesheetTemplateEngine {
	...
	private final LegacyHtml5TemplateParser parser = new LegacyHtml5TemplateParser(
			"LEGACYHTML5", 1);
	...
	public void setHttpServletRequest(MockHttpServletRequest request) {
	...
	private HtmlElements convertResultIntoDom(final String path,
			final String htmlString) {
		List<Node> nodes;
		if (isFragment(htmlString)) {
			nodes = parser.parseFragment(this.getConfiguration(), htmlString);
		} else {
			Document doc = parser.parseTemplate(this.getConfiguration(), path,
	...
	private boolean isFragment(final String htmlString) {
		return htmlString != null
				&& (!htmlString.contains("<!DOCTYPE")
						&& !htmlString.contains("<!doctype")
						&& !htmlString.contains("<html") && !htmlString
							.contains("<!HTML"));
	}
	...
	public String toString(final String path,
		...
		String fragment = this.process(path,
				getSpringWebContext(request, response, Locale.UK, model));
		return fragment;
	}
	...
	private IWebContext getSpringWebContext(final HttpServletRequest request,
		...
		return new SpringWebContext(request, response, servletContext, locale,
