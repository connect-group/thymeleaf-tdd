...
public class HasAttribute extends TypeSafeMatcher<HtmlElement> {
	...
	public void describeTo(Description description) {
		if(!testAttributeValue) {
			...
			description.appendText("has attribute ").appendValue(expectedAttributeName).appendText(" with value ").appendValue(expectedAttributeValue);
		}
	}

	@Override
	public boolean matchesSafely(HtmlElement item) {
		boolean hasNamedAttribute = item.getElement().getAttributeMap().containsKey(expectedAttributeName);
		if(hasNamedAttribute && testAttributeValue) {
			String actualAttributeValue = item.getElement().getAttributeMap().get(expectedAttributeName).getValue();
